...
INSERT INTO sanction_membre (reason, duration, user, author, cmd_id, serveur_id) VALUES
...
SELECT sm.* FROM sanction_membre AS sm INNER JOIN serveur ON serveur.id = serveur_id WHERE serveur_id = 1;

-- 07 - Récupérer la liste des joueurs dont les sanctions sont toujours actives sur un serveur Discord.
SELECT sm.user FROM sanction_membre AS sm INNER JOIN serveur ON serveur.id = serveur_id WHERE serveur_id = 1 AND duration <> NULL OR strftime('%s',duration) + strftime('%s', date) < strftime('%s', 'now');

-- 09 - Récupérer les sanctions liées à un joueur, et leur nombre.
SELECT * FROM sanction_membre WHERE user = 41771983423143937;
SELECT COUNT(*) FROM sanction_membre WHERE user = 41771983423143937 GROUP BY user;

-- 10 - Récupérer la liste de sanctions infligées à un joueur par un modérateur.
SELECT * FROM sanction_membre WHERE user = 41771983423143937 and author = 12771983423143989;

-- 11 - Récupérer la liste de sanctions infligées par un modérateur et leur nombre.
SELECT * FROM sanction_membre WHERE author = 12771983423143989;
SELECT COUNT(*) FROM sanction_membre WHERE author = 12771983423143989 GROUP BY author;
...
SELECT user FROM sanction_membre WHERE COUNT(serveur_id) > 1 GROUP BY user;

-- 16 - Récupérer la liste de joueur ayant des sanctions sur différents serveurs partageant une temporalité commune.
SELECT DISTINCT sm1.user FROM sanction_membre AS sm1
INNER JOIN sanction_membre AS sm2
...
SELECT user, type FROM sanction_membre INNER JOIN cmd ON cmd_id = cmd.id


SELECT DISTINCT sm1.user FROM sanction_membre AS sm1
INNER JOIN sanction_membre AS sm2 ON sm1.user = sm2.user AND sm1.id <> sm2.id
INNER JOIN cmd ON sm1.cmd_id